...
/// Ask the AI whether this email/message is a direct question requiring a human response.
/// Returns (is_question, explanation).
pub async fn is_question_for_user(text: &str, from: &str, subject: &str) -> Result<(bool, String)> {
...
Your job: determine if an incoming email or message is a direct question that requires a human response.
...
Rules:
- Newsletters, marketing, spam, and bulk emails are NOT questions (is_question: false)
- Social media digests, article summaries, and promotional content are NOT questions
...
- "Here is your daily briefing" → false
- "Subscribe to our newsletter" → false
- "EXTRA 25% off" → false
- "Spectrum Enterprise - Fiber Service Project" → false (marketing/sales)
- "The Briefing: OpenAI's Pitch to Businesses" → false (newsletter)
