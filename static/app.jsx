...
function App() {
  ...
  return (
    <>
      ...
      {focused && (
        <FocusErrorBoundary key={focused.id} notification={focused} onClose={() => setFocused(null)}>
          <window.Focus n={focused} nowMs={nowMs} onClose={() => setFocused(null)} onAnswer={onAnswer} onDismiss={onDismiss} onSnooze={(minutes) => apiSnooze(focused.id, minutes).then(() => { setFocused(null); }).catch((e) => { console.error('Snooze failed:', e); alert(e.message); })} />
        </FocusErrorBoundary>
      )}
